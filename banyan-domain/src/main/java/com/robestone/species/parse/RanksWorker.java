...
import com.robestone.species.Rank.RankType;
...
public class RanksWorker extends AbstractWorker {
	...
	void countRanksUsage() throws Exception {
		...
		Collection<String> latinWs = speciesService.findAllLatinNames();
		for (String latin : latinWs) {
			Entry e = speciesService.findEntryByLatinName(latin);
			if (e != null) {
	...
	void findWsRanksNotUsedByWd() throws Exception {
		...
		for (String latin : wdLatinToRank.keySet()) {
			...
			try {
				Entry entry = speciesService.findEntryByLatinName(latin);
				if (entry != null) {
