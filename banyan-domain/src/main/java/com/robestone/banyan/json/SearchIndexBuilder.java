...
//		sb.testListSize = 100;
	...
	public static class KeyEntry {
		...
		private void countLocals() {
			for (CandidateEntry match : topMatches) {
				// TODO HERE
				allDownstreamEntryIds.add(match.entry.getTaxonId());
				if (match.imageEntry != null) {
					allDownstreamEntryIdsWithImages.add(match.entry.getTaxonId());
				}
			}
	...
	public static class CandidateEntry implements Comparable<CandidateEntry> {
		TaxonNode entry;
		TaxonNode imageEntry;
		List<CandidateName> names = new ArrayList<CandidateName>();
		int score;
		
		public CandidateEntry() {
		}
		public CandidateEntry(TaxonNode entry, TaxonNode imageEntry) {
			this.entry = entry;
			...
			if (latin != null) {
				...
				this.addName(latin, entry.getLatinName(), clean, true, entry.getImage().getFilePath() != null);
			}
			
			String common = cleanSearchName(entry.getCommonName(), true);
			if (common != null) {
				String clean = cleanSearchName(entry.getCommonName(), false);
				this.addName(common, entry.getCommonName(), clean, false, entry.getImage().getFilePath() != null);
			}
	...
	void createCandidates() throws Exception {
		List<TaxonNode> entries = buildTree();
		List<CandidateEntry> entryNames = toCandidates(entries, getTaxonService());
...
//		}
	 ...
	 */
	public List<TaxonNode> buildTree() {
		JsonNode nroot = IndexPartitionsBuilder.buildTree(getTaxonService());
		if (testListSize > 0) {
			pruneTree(nroot);
		}
		
		Map<Integer, TaxonNode> map = new HashMap<>();
		buildTree(nroot, map);

		return new ArrayList<TaxonNode>(map.values());
	}
	private void pruneTree(JsonNode nroot) {
		List<JsonNode> current = new ArrayList<JsonNode>();
		int total = 1;
		current.add(nroot);
		while (true) {
			List<JsonNode> next = getAllChildren(current);
			current = next;
			total += next.size();
	...
	private List<JsonNode> getAllChildren(List<JsonNode> next) {
	...
	private TaxonNode buildTree(JsonNode node, Map<Integer, TaxonNode> map) {
		TaxonNode entry = getTaxonService().findTaxonById(node.getId());
		map.put(entry.getTaxonId(), entry);
		for (JsonNode child : node.getChildren()) {
			TaxonNode centry = buildTree(child, map);
			centry.setParent(entry);
		}
		return entry;
	}

	void addParents(CandidateEntry c, Tree<TaxonNode> tree, Map<Integer, CandidateEntry> ids) {
		Integer pid = c.entry.getParentId();
		if (pid != null && !ids.containsKey(pid)) {
			TaxonNode pe = tree.get(pid);
			CandidateEntry pc = toCandidateEntry(pe, getTaxonService());
			ids.put(pid, pc);
...
//		System.out.println("iterateOverKeys." + key.key);
	 ...
	 */
	public Taxon findBestMatchByQuery(String query) {
	...
	private static CandidateEntry toCandidateEntry(TaxonNode eentry, TaxonService speciesService) {
		TaxonNode ientry = null;
		// TODO this logic is no longer useful - it's collapsed down
		if (eentry.getImage() != null) {
			ientry = speciesService.findTaxonById(eentry.getImage().getEntryId());
		}
		CandidateEntry candidate = new CandidateEntry(eentry, ientry);
		return candidate;
	}
	private static List<CandidateEntry> toCandidates(List<TaxonNode> entries, TaxonService speciesService) {
	 ...
	 
{
	...
	private String toString(List<String> remoteKeys, Map<String, List<CandidateEntry>> localKeys) {
		...
		for (String localKey : localKeyList) {
			...
			for (CandidateEntry entry : some) {
				if (entryMap.put(entry.entry.getTaxonId(), entry) == null) {
					foundIds.add(entry.entry.getTaxonId());
				}
			}
		}
		if (!foundIds.isEmpty()) {
	...
	private LocalKey buildLocalKey(String key, Map<String, List<CandidateEntry>> localKeys) {
		...
		for (CandidateEntry ce : entries) {
			lkey.ids.add(ce.entry.getTaxonId());
		}
		return lkey;
	}
	...
	private void appendEntry(StringBuilder buf, CandidateEntry c) {
		buf.append("\t\t\"");
		buf.append(c.entry.getTaxonId());
		buf.append("\" : { \"ids\" : \"");
		String ids = TreeNodeUtilities.getCrunchedIdsForAncestors(c.entry);
...
//		buf.append(JsonParser.escape(c.matchedName));
		...
		if (imageEntry.getImage().getFilePath() != null) {
...
//					", imageLatin=" + imageEntry.getLatinName() + ", imageEntryId=" + imageEntry.getId());
			ImagesWorker.ImageInfo ii = ImagesWorker.toImageInfo(imageEntry);
