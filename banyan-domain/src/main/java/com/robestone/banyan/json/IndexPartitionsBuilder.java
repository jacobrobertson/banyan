...
public class IndexPartitionsBuilder {
	...
	public IndexPartitionsBuilder(TaxonService taxonService) {
		this.taxonService = taxonService;
	}
	public void partition(JsonNode node) {
	...
	private void assignPaths(JsonNode node) {
		String pathPart = node.getFileKey();
		List<JsonNode> byNodeId = new ArrayList<>(node.getChildren());
		Collections.sort(byNodeId, NodeIdComparator);
		int pos = 0;
		for (JsonNode child : byNodeId) {
...
//		System.out.println("createPaths." + node.getId() + "." + node.getFileKey());
	...
	private void assignSinglePartitions(JsonNode node) {
		node.getPartition().add(node);
		for (JsonNode child : node.getChildren()) {
			assignSinglePartitions(child);
		}
	}
	private void assignParents(JsonNode node) {
		for (JsonNode child : node.getChildren()) {
	...
	private void mergeMiscPartitions(JsonNode node) {
		...
		List<JsonNode> byPartitionSize = new ArrayList<>(node.getChildren());
		Collections.sort(byPartitionSize, PartitionSizeComparator);
		for (JsonNode child : byPartitionSize) {
	...
	private class PartitionSizeComparator implements Comparator<JsonNode> {
		@Override
	...
	private class NodeIdComparator implements Comparator<JsonNode> {
		@Override
		public int compare(JsonNode n1, JsonNode n2) {
	...
	private class Stats {
		...
		public Stats analyze(JsonNode node) {
			totalNodes++;
	...
	public void testAllNodesCanFindTheirPartition(JsonNode node) {
...
//				System.out.println("Found: " + message);
	...
	public Map<String, String> getAndAssignPartitionMap(JsonNode node) {
	...
	private void getAllPartitionKeys(JsonNode node, List<String> keys) {
	...
	private void assignPartitionPaths(JsonNode node, Map<String, String> map) {
		if (!node.getPartition().isEmpty()) {
	...
	public void partitionFromDB() throws Exception {
		...
		JsonNode root = buildTree(taxonService);
	...
	public void partitionFromFileSystem() throws Exception {
		JsonNode root = JsonFileUtils.parseRecursive(1);
		partitionAndSave(root);
	}
	public void partitionAndSave(JsonNode root) throws Exception {
	...
	public void outputPartitions(JsonNode node) throws Exception {
		if (!node.getPartition().isEmpty()) {
			String fileName = "p/" + node.getFilePath() + ".json";
			List<JsonEntry> entries = new ArrayList<>();
			for (JsonNode pn : node.getPartition()) {
				JsonEntry jentry = pn.getEntry();
				if (jentry == null) {
					TaxonNode eentry = taxonService.findTaxonById(pn.getId());
					jentry = JsonFileUtils.toJsonEntry(eentry, taxonService);
				}
				entries.add(jentry);
			}
	...
	public static JsonNode buildTree(TaxonService speciesService) {
		int[] count = {0};
		return buildNodeRecursively(1, 0, count, speciesService);
	}
	private static JsonNode buildNodeRecursively(Integer id, int depth, int[] count, TaxonService speciesService) {
		count[0]++;
		Collection<Integer> ids = speciesService.findChildrenIdsNonBoring(id);
		JsonNode node = new JsonNode(null, id, new ArrayList<>(ids));
		...
		for (Integer cid : ids) {
			JsonNode cnode = buildNodeRecursively(cid, depth + 1, count, speciesService);
