...
public class WikiDataService {
	...
	public WdTaxon findWdTaxon(String qID) {
	...
	public List<String> findAllTaxonQidsWithBrokenParentTaxonId() {
		List<String> found = template.query(
				"SELECT c.PARENT_QID FROM wd_taxons c " +
				"LEFT JOIN wd_taxons p " +
				"ON c.PARENT_QID = p.QID " +
				"WHERE c.PARENT_QID IS NOT NULL and p.qid IS NULL",
				new EntityMapperRowMapper(false));
		return found;
	}
	...
	private static class WdTaxonMapper implements ParameterizedRowMapper<WdTaxon> {
		public WdTaxon mapRow(ResultSet rs, int rowNum)
			...
			String latinName = getCleanLatinName(rs.getString("latin_name"));
	...
	private static String getCleanLatinName(String latinName) {
		return StringUtils.replaceChars(latinName, "[]", "()").trim() ;
	}
