...
import com.robestone.banyan.wikispecies.WikiSpeciesParser;
...
//		new ExtinctReporter().runEntry("Proganochelys", "Incertae sedis (Testudines)");
	...
	public void runAssignExtinct() throws Exception {
		...
		Collection<Entry> entries = getWikiSpeciesService().findEntriesForExtinctReport();
	...
	public void runEntry(String name, String parent) throws Exception {
		...
		if (matcher.find()) {
			...
			getWikiSpeciesService().assignExtinct(name);
		}
	}
