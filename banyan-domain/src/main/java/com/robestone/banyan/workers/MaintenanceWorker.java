...
import com.robestone.banyan.json.JsonWorker;
import com.robestone.banyan.json.SearchIndexBuilder;
import com.robestone.banyan.util.LogHelper;
import com.robestone.banyan.wikispecies.AuthWorker;
import com.robestone.banyan.wikispecies.Entry;
import com.robestone.banyan.wikispecies.ParseDoneChanger;
import com.robestone.banyan.wikispecies.RedirectWorker;
import com.robestone.banyan.wikispecies.WikiSpeciesCache;
import com.robestone.banyan.wikispecies.WikiSpeciesCrawler;
import com.robestone.banyan.wikispecies.WikiSpeciesTreeFixer;
...
public class MaintenanceWorker extends AbstractWorker {
	...
	public void runMaintenance() throws Exception {
		...
		new WikiSpeciesTreeFixer(getWikiSpeciesService()).run();
		
		// fix the parents (i.e. set ids for parent latin name) 
//		getWikiSpeciesService().fixParents();
		
		// cleanup extinct (might have added children of extinct parents)
//		getWikiSpeciesService().fixExtinct();
...
//			getWikiSpeciesService().recreateCleanNames();
		}
...
//		new LinkedImagesWorker().run();
	...
	private void addLatinNamesWithCommonNameAndNoImage(Set<String> names) throws Exception {
		Collection<Entry> entries = getWikiSpeciesService().findEntriesWithCommonNameAndNoImage();
