...
//		ic.fixOldBadImagesFromList();
	...
	public void deleteImagesWithNoEntry() {
		...
		for (Image image : images) {
			Entry entry = getWikiSpeciesService().findEntry(image.getEntryId());
			if (entry == null) {
	...
	public void removeBlackListedImages() {
		getWikiSpeciesService().udpateBlacklistedImages(BLACKLIST);
	}
	
	public void downloadAll(boolean onlyNew, boolean onlyTiny) throws IOException {
		System.out.println(">images.downloadAll");
		Collection<TaxonNode> links = getTaxonService().findAllImageFilePaths();
		int size = links.size();
		int count = 0;
		int chunk = 0;
		for (Taxon entry: links) {
			try {
	...
	public void downloadTests(String... latinNames) throws IOException {
		for (String latinName: latinNames) {
			Taxon entry = getTaxonService().findEntryByLatinName(latinName);
	...
	public void downloadOne(String latinName, int count, int size, boolean onlyNew, boolean onlyTiny) throws IOException {
		Taxon entry = this.getTaxonService().findEntryByLatinName(latinName);
		downloadOne(entry, count, size, onlyNew, onlyTiny);
	}
	public void downloadOne(Taxon entry, int count, int size, boolean onlyNew, boolean onlyTiny) throws IOException {
		ImageInfo info = toImageInfo(entry);
		
		String latinName = entry.getLatinName();
		String link = entry.getImage().getFilePath();
	...
	private boolean isThumbDownloaded(Taxon entry) {
		if (entry.getImage().getFilePath() == null) {
			return false;
		}
	...
	public static final String getImageFilePath(Taxon e, String type) {
	...
	public static ImageInfo toImageInfo(Taxon entry) {
		return new ImageInfo(entry.getLatinName(), entry.getImage().getFilePath());
	}
	...
	public static class ImageInfo {
		...
		public ImageInfo(final String latinName, final String imageLink) {
			
			this.latinNameClean = getCleanLatinName(latinName);
			
			this.fileHashDir = getImagePathHashed(latinNameClean);
			
			String fileImageLink = parseFileName(imageLink);
			int pos = fileImageLink.lastIndexOf('/');
			String fileName = fileImageLink.substring(pos + 1);

			String name = fileName.toUpperCase();
			
			if (name.endsWith(".SVG") || name.endsWith(".WEBP") || name.endsWith(".XCF")) {
				this.urlExtraFileTypeToken = "";
				this.urlExtraFileExtension = "png";
				...
				this.urlExtraFileTypeToken = "";
				this.urlExtraDash = true;
				this.urlExtraFileExtension = "jpg";
			} else if (name.endsWith(".TIF") || name.endsWith(".TIFF")) {
				this.urlExtraFileTypeToken = "lossy-page1-";
				this.urlExtraFileExtension = "jpg";
			} else if (name.endsWith(".PDF") || name.endsWith(".DJVU")) {
				// https://commons.wikimedia.org/wiki/File:Flora_Graeca,_Volume_8.djvu?page=95
				String page = getPageTokenFromLink(imageLink);
				if (page == null) {
					page = "page1-";
				}
				this.urlExtraFileTypeToken = page;
				this.urlExtraFileExtension = "jpg";
			} else {
				this.urlExtraFileTypeToken = "";
				this.urlExtraFileExtension = null;
			}

			this.fileExtension = getExtensionFromLink(fileName);
			
			this.urlBasePath = parseFileName(imageLink);
		}
		...
		public String getFilePathRelative() {
			...
			String iconFileName = fileHashDir + "/" + latinNameClean + thumbFileExtension;
			return iconFileName;
		}
