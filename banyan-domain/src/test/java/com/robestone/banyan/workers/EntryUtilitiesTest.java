...
public class EntryUtilitiesTest extends TestCase {
	...
	public void testTreeReport() {
		...
		Tree<Entry> t = createMockTree();
		
		List<Tree<Entry>> trees = EntryUtilities.findDisconnectedTrees(t);
	...
	private Tree<Entry> createMockTree() {
	...
	private AnalyzableTreeNode addEntry(List<Entry> list, int c, int p) {
		Entry e = new Entry();
		e.setId(c);
	...
	public void testCollapseListToOne() {
		...
		((AnalyzableTreeNode)list.get(3)).setCommonName("L3");
		e = EntryUtilities.collapseListToOne(list);
		assertEquals("L3", e.getLatinName());
		
		((AnalyzableTreeNode)list.get(4)).setCommonName("L4");
		e = EntryUtilities.collapseListToOne(list);
		assertEquals("L3", e.getLatinName());
		
		((AnalyzableTreeNode)list.get(4)).setCommonName("z L 4 a");
		e = EntryUtilities.collapseListToOne(list);
