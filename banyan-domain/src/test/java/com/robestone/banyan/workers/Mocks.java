...
//		System.out.println("addIds." + seed + ":" + root.getLatinName() + "/" + root.getCommonName());
	...
	public AnalyzableTreeNode getEntryForId(int id) {
		return EntryUtilities.findEntry(getRoot(), id);
	}
	public AnalyzableTreeNode getEntryForName(String name) {
		Entry root = getRoot();
		AnalyzableTreeNode found = findForName(name, root);
		if (found == null) {
			found = root;
		}
		return found;
	}
	private AnalyzableTreeNode findForName(String n, Entry e) {
		...
		if (e.hasChildren()) {
			for (Entry c: e.getChildren()) {
				AnalyzableTreeNode f = findForName(n, c);
				if (f != null) {
					return f;
				}
			}
